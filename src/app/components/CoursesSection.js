...
export default function CoursesSection() {
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden">
      <section id="about" className={`absolute inset-0 swipe-transition select-none ${isCoursesVisible ? '-translate-x-full' : 'translate-x-0'}`}>
        <AboutSection/>
      </section>

      <section id="courses" className={`absolute inset-0 swipe-transition transform select-none ${isCoursesVisible ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="min-h-screen flex items-center justify-center bg-gray-100 py-6 sm:py-12 lg:py-16">
          <div className="container mx-auto px-4">
            <div className="text-center mb-10 sm:mb-14" data-aos="fade-up" data-aos-duration="1000">
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-blue-600">Courses We Offer</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 pb-6">
              {[
                ...
                <div key={course.title} className="w-[90%] sm:w-full mx-auto bg-white bg-opacity-90 shadow-lg rounded-lg p-3 sm:p-5 text-center flex flex-col items-center justify-center transition-transform transform hover:scale-105 hover:-translate-y-1 hover:shadow-xl" data-aos="fade-up" data-aos-delay={course.delay}>
      ...
      <button
        onClick={toggleSwipe}
        className={`absolute top-1/2 ${isCoursesVisible ? 'left-4' : 'right-4'} z-10 bg-blue-600 text-white p-3 rounded-full shadow-lg hover:bg-blue-700 transition-colors duration-300 w-10 h-10 flex items-center justify-center text-lg sm:w-12 sm:h-12 sm:text-2xl font-bold`}
